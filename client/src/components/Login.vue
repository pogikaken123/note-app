    ...
    <div v-if="IsAuthenticated == false">
      ...
      <div>
        <form>
          ...
          <div>
            <button class="button2" @click.prevent="addNotes">
              Add new notes
            </button>
          </div>
        </form>
      ...
      <ul v-if="isNotShow">
        <h3 v-for="notes in data.data.data" :key="notes._id">
          Notes Description:{{ notes.description }} || Completed:{{
            notes.isCompleted
          }}
          <button class="remove" @click="deleteNotes(notes._id)">X</button>
          <button class="button1" @click="reverseMark(notes)">
      ...
      <div v-if="completed">
        <ul v-for="notes in data.data.data" :key="notes._id">
          <h1 v-if="!notes.isCompleted">
            {{ notes.description }}:{{ notes.isCompleted }}
            <button class="remove" @click="deleteNotes(notes._id)">X</button>
            <button class="button1" @click="reverseMark(notes)">
              Mark Complete/Incomplete
            </button>
...
export default {
  ...
  methods: {
    ...
    updateForms() {
      axios
        .put(
          "http://localhost:5000/api/v1/notes/" + this.id,
          {
            title: this.title,
    ...
    viewAllNotes() {
      axios
        ...
        .then(response => {
          (this.data = response), (this.completed = false);
          (this.note = false), (this.isNotShow = !this.isNotShow);
        });
    },
    ...
    reverseMark(notes) {
      axios
        .put(
          "http://localhost:5000/api/v1/notes/" + notes._id,
          {
